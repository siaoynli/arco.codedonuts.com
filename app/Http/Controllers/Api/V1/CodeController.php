    ...
    public function send(Request $request)
    {
        ...
        if (newCache("api")->has($day_limit_key)) {
            ...
            if (newCache("api")->get($day_limit_key) >= 10) {
                return responseJsonMessage("该手机号码发送短信超过单日限制,请联系管理员");
            }
        ...
        if ($value) {
            ...
            if ($interval_time > 0) {
                return responseJsonMessage("该手机号码已经发过短信，请过" . $interval_time . '秒后再试');
            }
        }
        //生成验证码
        $code = str_pad((string)rand(1, 9999), 6, "0", STR_PAD_LEFT);
        ...
        //手机号码分钟频率限制
        newCache("api")->put($send_limit_key, time() + 60, now()->addSecond(60));

        //后台发送验证码
        dispatch(new AliSmsQueue((int)$phone, AliSms::codeMessage($code)))->onQueue("sms");

        return responseJsonData(["key" => $key, "expire_at" => $expireAt->toDateTimeString()]);
